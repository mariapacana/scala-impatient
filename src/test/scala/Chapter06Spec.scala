...
class Chapter06Spec extends FlatSpec with Matchers {
  ...
  "Conversions" should "convert units using separate methods" in {
    Conversions.inchesToCentimeters(1) shouldBe 2.54
    Conversions.gallonsToLiters(1) shouldBe 3.79
    Conversions.milesToKilometers(1) shouldBe 1.61
  }
  ...
  "UnitConversion" should "convert units using general super-class" in {
    ...
    inchesToCentimeters.convert(1) shouldBe 2.54

    val gallonsToLiters: UnitConversion = GallonsToLiters
    gallonsToLiters.convert(1) shouldBe 3.79

    val milesToKilometers: UnitConversion = MilesToKilometers
    milesToKilometers.convert(1) shouldBe 1.61
  }
  ...
  it should "implement isRed function" in {
    CardColors.check(PlayingCard.Clubs) shouldBe false
    CardColors.check(PlayingCard.Diamonds) shouldBe true
    CardColors.check(PlayingCard.Hearts) shouldBe true
    CardColors.check(PlayingCard.Spades) shouldBe false
  }
