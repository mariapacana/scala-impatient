...
class Chapter13Spec extends FlatSpec with Matchers {
  ...
  "indexes" should "produce a mutable map of mutable set of indexes of all characters" in {
    //when
    val result: mMap[Char, mSet[Int]] = indexes("Mississippi")

    //then
    result('M') shouldBe mSet(0)
    result('i') shouldBe mSet(1, 4, 7, 10)
    result('s') shouldBe mSet(2, 3, 5, 6)
    result('p') shouldBe mSet(8, 9)
  }
  ...
  "reverseList" should "reverse the given list" in {
    //given
    val lst = List(1, 2, 3, 4, 5)

    //when
    val result: List[Int] = reverseList(lst)
